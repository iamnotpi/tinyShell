...
cmd commands[] = {
    ...
    {"child", "Start a child process.", "child [-bg, optional] [args, optional] : Start child process(es). \nIf the argument '-bg' is provided as the second argument, start all specified process in background mode.\nIf not, start the first process appears in the command line in foreground mode.\nNote: 'child' can start multiple (non-UWP) .exe file by using 'child -bg <file1.exe> <file2.exe>....\nBy default, if there is no argument provided, 'child' starts a simple foo.exe process (associated with foo.c) in the appropriate mode. ", childprocess},
